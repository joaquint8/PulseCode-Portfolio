...
export function ImagesBadge({
  ...
  return (
    <Component
      ...
      onMouseEnter={() => forceHovered === undefined && setInternalHovered(true)}
      onMouseLeave={() => forceHovered === undefined && setInternalHovered(false)}
    >
      <div className="relative" style={{ width: folderSize.width, height: folderSize.height, transformStyle: "preserve-3d" }}>
        ...
        {displayImages.map((image, index) => {
          ...
          const teaseY = -4 - (totalImages - 1 - index);
          ...
          return (
            <div
              ...
              style={{
                ...
                transform: `translateX(calc(-50% + ${isHovered ? hoverX : 0}px)) translateY(${isHovered ? hoverY : teaseY}px) rotate(${isHovered ? baseRotation : teaseRotation}deg)`,
                transition: `transform 300ms ease ${index * 30}ms, width 300ms ease ${index * 30}ms, height 300ms ease ${index * 30}ms`
              }}
            >
              <img src={image} alt={`Preview ${index + 1}`} loading="lazy" decoding="async" className="h-full w-full object-cover" />
            </div>
          );
        })}
        <div
          ...
          style={{
            ...
            zIndex: 20,
            transform: `rotateX(${isHovered ? -45 : -25}deg) scaleY(${isHovered ? 0.8 : 1})`,
